# loops -> sometimes we may need to do a piece of work a number of repeated times and in such cases we use loops
# Aloop is a contro. structure that allows us to execute a block of code repeatedly intil a certain condition is met.
# Threre are two types of loops in python : The  for loop and the while loop.

# Below is the syntax of a for loop in python:
"""
for variable in range(n):
    # block of code to be executed
"""
for greeting in range (5):
    print("Hello Moses",greeting)

print('================')

for number in range(10,20):
    print(number) 

print('============')    
#  find the even numbers in range 50 to 71
for number in range (50,71,2):
    print(number)


print('============')
# create a python program that prints the odd numbers from 100 to 150
for number in range(101,150,2):
    print(number) 


print('================')
# create a program that prints the multiples of 3 starting from 201 to 150
for  number in range(201,149,-3):
    print(number)



print('================')
# create a python program that prints  the leap years between 2000 and 2024

for number in range(2000,2024,4):
      print(number)  




